      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {fridgeProducts.map((fridge) => (
            <div key={fridge.id} className="bg-white p-6 rounded-lg shadow-md">
              <Image
                src={"https://www.furniture-king.co.za/wp-content/uploads/2022/07/H125RTS-2_sq.png"}
                alt={fridge.name}
                width={500}
                height={500}
                className="w-full h-48 object-cover mb-4 rounded-lg"
              />
              <h3 className="text-xl font-semibold mb-2">{fridge.name}</h3>
              <p className="text-gray-700">{fridge.description}</p>
            </div>
          ))}
        </div>
      </div>
